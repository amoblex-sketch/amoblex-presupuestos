...
const USERS = [
  { id:"guille", nombre:"Guille", rol:"admin", pin:"1234" },
  { id:"manu", nombre:"Manu", rol:"admin", pin:"1234" },
  { id:"florS", nombre:"Flor S", rol:"vendedora", pin:"1234" },
  { id:"florP", nombre:"Flor P", rol:"vendedora", pin:"1234" },
];
        ...
        {!loginUser ? (
          ...
          <div>
            ...
            <div style={{background:"#1a1a1a",borderRadius:14,padding:24,border:"1px solid #222"}}>
              ...
              <div style={{fontSize:11,color:"#888",marginBottom:8}}>Ingresá tu PIN</div>
              <div style={{display:"flex",justifyContent:"center",gap:6,marginBottom:12}}>
                {[0,1,2,3].map(i=>(
                  <div key={i} style={{width:38,height:44,borderRadius:8,border:`2px solid ${loginPin.length>i?"#c9a96e":"#333"}`,background:loginPin.length>i?"#222":"#181818",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:700,color:"#fff"}}>
                    {loginPin[i]?"●":""}
                  </div>
                ))}
              </div>
              <input 
                ...
                value={loginPin} 
                onChange={e=>{const v=e.target.value.replace(/\D/g,"").slice(0,4);setLoginPin(v);setLoginErr("");}}
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,maxWidth:200,margin:"0 auto"}}>
                {[1,2,3,4,5,6,7,8,9,null,0,"⌫"].map((n,i)=>(
                  <button key={i} onClick={()=>{
                    if(n===null) return;
                    if(n==="⌫") { setLoginPin(p=>p.slice(0,-1)); return; }
                    if(loginPin.length<4) { const np=loginPin+n; setLoginPin(np); if(np.length===4) setTimeout(()=>{const usr=USERS.find(x=>x.id===loginUser.id);doLogin(usr,np);},150); }
                  }} style={{padding:"10px 0",borderRadius:8,border:"1px solid #333",background:n===null?"transparent":"#222",fontSize:16,fontWeight:500,cursor:n===null?"default":"pointer",fontFamily:"'DM Sans',sans-serif",color:"#fff",visibility:n===null?"hidden":"visible"}}>
                    {n}
                  </button>
                ))}
              </div>
              {loginErr && <div style={{marginTop:10,padding:"6px 12px",borderRadius:6,background:"#3b1111",color:"#f87171",fontSize:12,fontWeight:600}}>{loginErr}</div>}
              {loginPin.length===4 && !loginErr && <button onClick={()=>doLogin(loginUser)} style={{marginTop:12,width:"100%",padding:"10px",borderRadius:8,border:"none",background:"#fff",color:"#111",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Ingresar</button>}
            </div>
          </div>
        )}
      </div>
